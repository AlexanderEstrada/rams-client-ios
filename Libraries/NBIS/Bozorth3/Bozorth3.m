...
@interface Bozorth3(){
    ...
    int gct[ GCT_SIZE ];
    int ctt[ CTT_SIZE ];
    int ctp[ CTP_SIZE_1 ][ CTP_SIZE_2 ];
    int yy[ YY_SIZE_1 ][ YY_SIZE_2 ][ YY_SIZE_3 ];
...
- (int)bozorth_probe_init:(struct xyt_struct *)pstruct{
    ...
    [self bz_comp:pstruct->nrows
                xcol:pstruct->xcol
                ycol:pstruct->ycol
            thetacol:pstruct->thetacol
        ncomparisons:&sim cols:scols colptrs:scolpt];
    
    msim = sim;	/* Init search to end of Subject's pointwise comparison table (last edge in Web) */
    
    [self bz_find:&msim colpt:scolpt];
    
    if ( msim < FDD ) msim = ( sim > FDD ) ? FDD : sim;
...
- (int)bozorth_gallery_init:(struct xyt_struct *)gstruct{
    ...
    [self bz_comp:gstruct->nrows
             xcol:gstruct->xcol
             ycol:gstruct->ycol
         thetacol:gstruct->thetacol
     ncomparisons:&fim
             cols:fcols
          colptrs:fcolpt];
    
    mfim = fim;
    [self bz_find:&mfim colpt:fcolpt];
    
    if ( mfim < FDD )	mfim = ( fim > FDD ) ? FDD : fim;
    
    return mfim;
}

- (int)computeBozortScore:(struct xyt_struct *)pstruct gstruct:(struct xyt_struct *)gstruct{
    int probe_len = [self bozorth_probe_init:pstruct];
    int gallery_len = [self bozorth_gallery_init:gstruct];
    int np = [self bz_match:probe_len gallery:gallery_len];
    int ms = [self bz_match_score:np pstruct:pstruct gstruct:gstruct];
    
    return ms;
}

- (void)bz_comp:(int)npoints
           xcol:(int *)xcol
           ycol:(int *)ycol
       thetacol:(int *)thetacol
    ncomparisons:(int *)ncomparisons
           cols:(int[][COLS_SIZE_2])cols
        colptrs:(int *[])colptrs{
...
- (void)bz_find:(int *)xlim colpt:(int *[])colpt{
    int midpoint;
    int top;
    int bottom;
    int state;
...
- (void)rtp_insert:(int *[])rtpx l:(int)l idx:(int)idx ptr:(int *)ptr{
    int shiftcount;
    int ** r1;
    int ** r2;
...
- (int)bz_match:(int)probe_ptrlist_len gallery:(int)gallery_ptrlist_len{
    int i;			/* Temp index */
    int ii;			/* Temp index */
    ...
    for ( k = 1; k < probe_ptrlist_len; k++ ) {
        ...
        for ( j = st; j <= gallery_ptrlist_len; j++ ) {
            ...
            if ( n == 1 )
                ++l;
            [self rtp_insert:rtp l:l idx:edge_pair_index ptr:&rot[edge_pair_index][0]];
            ++edge_pair_index;
...
- (int)bz_match_score:(int)np
              pstruct:(struct xyt_struct *)pstruct
              gstruct:(struct xyt_struct *)gstruct{
    int kx, kq;
    int ftt;
    int tot;
    int qh;
    ...
    if ( pstruct->nrows < min_computable_minutiae ) {
        ...
        if ( gstruct->nrows < min_computable_minutiae ) {
            if ( verbose_bozorth )
                NSLog(@"bz_match_score(): both probe and gallery file have too few minutiae to compute a real Bozorth match score");
        } else {
            if ( verbose_bozorth )
                NSLog(@"bz_match_score(): probe file has too few minutiae to compute a real Bozorth match score");
        }
#endif
...
#ifndef NOVERBOSE
        if ( verbose_bozorth )
            NSLog(@"bz_match_score(): gallery file has too few minutiae to compute a real Bozorth match score");
#endif
        return ZERO_MATCH_SCORE;
    }
    ...
    for ( k = 0; k < np - 1; k++ ) {
        ...
        do {
            ...
            do {
                ...
                l  = colp[kx][4];
                kx++;
                [self bz_sift:&ww kz:kz qh:&qh l:l kx:kx ftt:ftt tot:&tot overflow:&qq_overflow];
                if ( qq_overflow ) {
                ...
                for ( i = kq; i < np; i++ ) {
                    ...
                    if ( z == 3 ) {
                        ...
                        if ( z != colp[k][1] && l != colp[k][3] ) {
                            kx = i + 1;
                            [self bz_sift:&ww kz:z qh:&qh l:l kx:kx ftt:ftt tot:&tot overflow:&qq_overflow];
                            if ( qq_overflow ) {
                                return QQ_OVERFLOW_SCORE;
                            }
                        }
...
//                l = 1;
    ...
    match_score = [self bz_final_loop:tp];
    return match_score;
}
