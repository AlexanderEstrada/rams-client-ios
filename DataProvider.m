...
- (instancetype)initWithPageSize:(NSUInteger)pageSize initWithTotalData:(NSUInteger)total withEntity:(NSString *) entity andSort:(NSString *)sort basePredicate:(NSPredicate *)basePredicate{
    ...
    if (self) {
        ...
        _pagedArray = [[AWPagedArray alloc] initWithCount:total objectsPerPage:pageSize];
        _pagedArray.delegate = self;
        _dataLoadingOperations = [NSMutableDictionary dictionary];
        _operationQueue = [NSOperationQueue new];
        if (sort) _sortCriteria = sort;
        _entityName = entity;
        if (basePredicate) _basePredicate = basePredicate;
    }
    return self;
}
...
- (NSMutableArray *)dataObjects {
    return (NSMutableArray *)_pagedArray;
}
...
//    DataLoadingOperation *operation = [[DataLoadingOperation alloc] initWithIndexes:indexes];
    DataLoadingOperation * operation = [[DataLoadingOperation alloc] initWithIndexes:indexes withEntity:_entityName sortDescriptorWithKey:_sortCriteria basePredicate:_basePredicate];
